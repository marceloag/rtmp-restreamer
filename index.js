...
app.post('/', (req, res) => {
    ...
    var igUrl = `"rtmps://live-upload.instagram.com:443/rtmp/${req.body.key}"`
      ...
      // const child = spawn('ffmpeg', ['-rtbufsize','256M', '-re', '-loop', '1', '-i' , 'bg.jpeg', '-i', req.body.URL,'-filter_complex', '"[0:v]scale=1080:-2[bg];[bg][1:v]overlay=(main_w-overlay_w)/2:(main_h-overlay_h)/2"', '-c:a', 'aac', '-ar', '44100', '-b:a', '96k', '-pix_fmt', 'yuv420p', '-profile:v', 'baseline', '-s', '720x1280', '-bufsize', '6000k', '-vb', '400k', '-maxrate', '3000k', '-deinterlace', '-vcodec', 'libx264', '-preset', 'slower', '-g', '30', '-r', '30', '-f', 'flv', igUrl], {shell: true});
